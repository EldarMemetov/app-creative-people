...
const FALLBACK_LIFETIME_MS = 1000 * 60 * 15;
  ...
  persist(
    (set, get) => ({
      ...
      init: async () => {
        ...
        await get().refresh();
        if (get().accessToken) {
          api.defaults.headers.Authorization = `Bearer ${get().accessToken}`;
          await get().fetchUser();
        } else {
      ...
      login: async (email, password) => {
        ...
        try {
          ...
          const expiresAt = res?.expiresAt || Date.now() + FALLBACK_LIFETIME_MS;

          set({ accessToken: token, expiresAt, loading: false });
          ...
          if (status === 401) {
            ...
            if (newToken) {
              ...
              try {
                ...
                set({ user: user2, isAuthChecked: true });
                return user2;
              } catch (_) {}
            }
          }
      ...
      refresh: async () => {
        ...
        const promise = (async () => {
          try {
            ...
            const token = res.token;
            const expiresAt =
              res.expiresAt || Date.now() + FALLBACK_LIFETIME_MS;
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          if (state.accessToken) {
            api.defaults.headers.Authorization = `Bearer ${state.accessToken}`;
          }
          setTimeout(() => {
